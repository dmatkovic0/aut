...
async function addPrehireChecklist(driver) {
  try {
    ...
    const prehireChecklistButton = await driver.wait(
      until.elementLocated(
        By.xpath("//label[normalize-space()='Pre-hire Checklist']")
      ),
      10000
    );
    ...
    console.error(
      "❌ Failed to add employee with Pre-hire Checklist:",
      error.message
    );
    throw error;
  }
...
async function addNoAutoAssignment(driver) {
  try {
    ...
    const noAutoAssignmentButton = await driver.wait(
      until.elementLocated(
        By.xpath("//label[normalize-space()='No Auto Assignment']")
      ),
      10000
    );
    ...
    console.error(
      "❌ Failed to add employee with No Auto Assignment:",
      error.message
    );
    throw error;
  }
